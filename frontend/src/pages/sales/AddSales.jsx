...
const groupedSales = {}; //create object which store invoice number 
for (const item of sales) {
  if (!groupedSales[item.invoiceNo]) {   //checks that for that particular invoice the array is there or not 
    groupedSales[item.invoiceNo] = [];//if not create one 
  }
  groupedSales[item.invoiceNo].push(item); //if there is then item to that array
}
            ...
            <div className="row mb-2" key={index}>
              <div className="col">
                <select className="form-control" value={item.product_id} onChange={e => handleProductChange(index, "product_id", e.target.value)}>
                  ...
                  {products .filter(p => p.stock > 0 || p.pid.toString() === item.product_id) // keep only available OR the one already selected
                  .map(p => (
                    <option key={p.pid} value={p.pid.toString()}>
